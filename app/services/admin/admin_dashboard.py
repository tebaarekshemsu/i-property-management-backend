...
def get_dashboard_data(admin_id: int, db: Session):
    pending_reports = db.query(Invitation).filter(Invitation.admin_id == admin_id, Invitation.status == "not seen").count() or 0
    total_houses = db.query(House).filter(House.assigned_for == admin_id).count() or 0
    pending_visits = db.query(Invitation).filter(Invitation.admin_id == admin_id, Invitation.status == "not seen").count() or 0
