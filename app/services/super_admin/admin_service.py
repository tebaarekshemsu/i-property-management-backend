...
# Handler for adding a new location (area)
def add_location(location_data):
    db = SessionLocal()
    try:
        area_code_str = location_data.get("area_code")
        name = location_data.get("name")

        if not area_code_str or not name:
            return {"error": "Area code and name are required"}, 400

        try:
            area_code_int = int(area_code_str)
        except ValueError:
            return {"error": "Area code must be a valid integer."}, 400

        # Check for existing area code or name
        existing_area = db.query(Area).filter(
            (Area.code == area_code_int) | (Area.name == name)
        ).first()
        if existing_area:
            return {"error": "Location with this area code or name already exists."}, 409

        new_area = Area(code=area_code_int, name=name)
        db.add(new_area)
        db.commit()
        db.refresh(new_area)
        return {"message": "Location added successfully", "location": {"area_code": new_area.code, "name": new_area.name}}, 201
    except IntegrityError:
        db.rollback()
        return {"error": "A location with this area code or name already exists (integrity conflict)."}, 409
    except Exception as e:
        db.rollback()
        print(f"Error adding location: {e}")
        return {"error": f"Failed to add location: {str(e)}"}, 500
    finally:
        db.close()